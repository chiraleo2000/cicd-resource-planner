...
const JOB_SPECS = [
  ...
  ['push-packages', 'store',
    ...
    ['compile', 'sbom'], 'build', null, ['G-10'],
    'Publish Maven/npm/PyPI packages + SBOM to artifact repository',
    ['if [ -f pom.xml ]; then mvn -B deploy -DskipTests -DaltDeploymentRepository=internal::default::${NEXUS_URL}/repository/maven-releases/; fi',
...
function jobsCsv(ir) {
  return ir.jobs.filter(j => j.enabled).map(j => j.id).join(',');
}

function usagesCsv(ir) {
  return ir.jobs.filter(j => j.enabled)
    .map(j => j.id + ':' + (j.tool_id || 'bootstrap')).join(',');
}
...
function header(ir, flavor) {
  ...
  return '# Generated by CI/CD Resource Planner ' + PIPE_SCHEMA + '\n'
    ...
    + '# jobs=' + jobsCsv(ir) + '\n'
    + '# usages=' + usagesCsv(ir) + '\n'
    + '# Set REGISTRY, NEXUS_URL, DEV_URL, UAT_URL, PROD_URL, DR_URL in CI variables.\n'
    + '# Host install: copy install/<vm>.sh to each VM, or run install/all.sh.\n';
}

function appName(ir) {
  const pj = ir.project || {};
  return sanitizeVm(pj.name || 'app').toLowerCase() || 'app';
}

function vmCsv(ir) {
  const names = (ir.vms || []).map(v => sanitizeVm(v.name || 'VM'));
  return names.length ? names.join(',') : '-';
}

function envMap(ir) {
  const app = appName(ir);
  return {
    APP_NAME: app,
    IMAGE: app,
    PROFILE: ir.profile,
    ORCHESTRATOR: ir.orchestrator,
    ENV_CHAIN: ir.envs.join(','),
    CICD_TOOLS: ir.tools.join(','),
  };
}
...
export function emitGitlab(ir) {
  ...
  L.push('');
  L.push('variables:');
  L.push('  APP_NAME: "' + env.APP_NAME + '"');
  L.push('  IMAGE: "' + env.IMAGE + '"');
  L.push('  PROFILE: "' + env.PROFILE + '"');
  L.push('  ORCHESTRATOR: "' + env.ORCHESTRATOR + '"');
  L.push('  ENV_CHAIN: "' + env.ENV_CHAIN + '"');
  L.push('  CICD_TOOLS: "' + env.CICD_TOOLS + '"');
  L.push('  REGISTRY: "${CI_REGISTRY_IMAGE}"');
  L.push('  NEXUS_URL: "${NEXUS_URL}"');
  L.push('  DEV_URL: "${DEV_URL}"');
  L.push('  UAT_URL: "${UAT_URL}"');
  L.push('  PROD_URL: "${PROD_URL}"');
  L.push('  DR_URL: "${DR_URL}"');
  L.push('  IMAGE_REF: "${REGISTRY}/${IMAGE}:${CI_COMMIT_SHORT_SHA}"');
  L.push('');
  for (const j of ir.jobs) {
    if (!j.enabled) continue;
    L.push(j.id + ':');
    ...
    if (j.tool_id) L.push('  # tool: ' + j.tool_id);
    L.push('  script:');
...
export function emitGithub(ir) {
  ...
  const L = [
    ...
    '  DR_URL: ${{ vars.DR_URL }}',
    '  IMAGE_REF: ${{ vars.REGISTRY }}/' + env.APP_NAME + ':${{ github.sha }}',
    '',
    'jobs:',
  ];
  for (const j of ir.jobs) {
    ...
    if (j.needs.length) L.push('    needs: [' + j.needs.join(', ') + ']');
    if (j.when === 'manual') {
      L.push("    if: github.ref == 'refs/heads/main'");
      L.push('    environment: ' + (j.env || j.id));
    } else if (j.env) {
      L.push('    environment: ' + j.env);
    }
    if (j.tool_id) L.push('    # tool: ' + j.tool_id + ' env=' + (j.env || '-'));
    L.push('    steps:');
    ...
    const script = (j.script && j.script.length) ? j.script : ['true'];
    L.push('        run: |');
    for (const s of script) L.push('          ' + s);
    L.push('');
  }
  return L.join('\n') + '\n';
}
...
export function emitAzure(ir) {
  ...
  for (const st of ir.stages) {
    ...
    L.push('- stage: ' + st.id);
    L.push('  displayName: ' + st.label);
    L.push('  jobs:');
    for (const j of jobs) {
      L.push('  - job: ' + j.id.replace(/-/g, '_'));
      L.push('    displayName: ' + j.name);
      if (j.tool_id) L.push('    # tool: ' + j.tool_id);
      L.push('    steps:');
      const script = (j.script && j.script.length) ? j.script : ['true'];
      L.push('    - script: |');
      for (const s of script) L.push('        ' + s);
      L.push('      displayName: ' + j.id);
    }
    L.push('');
  }
  return L.join('\n') + '\n';
...
export function emitJenkins(ir) {
  ...
  for (const st of ir.stages) {
    const jobs = ir.jobs.filter(j => j.enabled && j.stage === st.id);
    if (!jobs.length) continue;
    L.push("    stage('" + st.label + "') {");
    L.push('      steps {');
    for (const j of jobs) {
      if (j.tool_id) L.push("        echo 'tool=" + j.tool_id + " job=" + j.id + "'");
      for (const s of j.script) L.push("        sh '" + s.replace(/'/g, '"') + "'");
    }
    L.push('      }');
    L.push('    }');
  }
...
function vmScriptBody(byId, ids, vmName, role, ir) {
  const pkgs = [];
  const body = [];
  const selfHost = [];
  const managed = [];
  ...
  const lines = [
    ...
    '# VM: ' + vmName,
    '# Role: ' + (role || ''),
    '# Tools: ' + (ids.length ? ids.join(' ') : '-'),
    '# Self-host: ' + (selfHost.length ? selfHost.join(' ') : '-'),
    '# Managed: ' + (managed.length ? managed.join(' ') : '-'),
    '# Envs: ' + (((ir && ir.envs && ir.envs.length) ? ir.envs.join(' ') : '-') ),
    '# Profile: ' + ((ir && ir.profile) || '-'),
    'HERE="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"',
    '# shellcheck disable=SC1091',
    '. "$HERE/00-common.sh"',
    'log "ติดตั้งเครื่องมือบน ' + vmName + '"',
  ];
  if (pkgs.length) lines.push('pkg_install ' + pkgs.join(' '));
  return lines.concat(body).concat(['log "เสร็จบน ' + vmName + '"', '']).join('\n');
}
...
export function buildInstallPack(ir, tools) {
  ...
  vms.forEach((vm, i) => {
    ...
    files[fname] = vmScriptBody(byId, ids, vm.name || name, vm.role || '', ir);
  });
  const leftover = [...(ir.tools || [])].filter(t => !assigned.has(t));
  const extra = [];
  if (leftover.length) {
    const label = vmFiles.length ? 'unassigned' : 'selected-tools';
    const fname = 'install/' + label + '.sh';
    extra.push(fname);
    files[fname] = vmScriptBody(
      byId, leftover, label,
      vmFiles.length
        ? 'เครื่องมือที่เลือกแล้วยังไม่ถูกจัดลง VM'
        : 'ติดตั้งตามเครื่องมือที่เลือก (ยังไม่มีผัง VM)',
      ir);
  }
  const all = [
    '#!/usr/bin/env sh',
    ...
    '# shellcheck disable=SC1091',
    '. "$HERE/00-common.sh"',
    'log "ติดตั้งทุกเครื่องตามผังทรัพยากร"',
  ];
  vmFiles.concat(extra).forEach(f => {
    all.push('sh "$HERE/' + f.slice('install/'.length) + '"');
  });
  if (!vmFiles.length && !extra.length) {
    all.push('log "ยังไม่มี VM และยังไม่ได้เลือกเครื่องมือ — จัดในแท็บวางแผนก่อน"');
  }
  all.push('log "ติดตั้งครบแล้ว"');
  all.push('');
