...
T("gitlab-ce", "GitLab Community Edition (Self-hosted Git)", 1, "Git Repository",
  ...
  "ทางเลือกหลักของภาครัฐที่ต้องการ Git แบบ On-premise เต็มรูปแบบ กินทรัพยากรมากเพราะรวมหลาย service; ถ้าต้องการเบาให้ใช้ Gitea; Package Registry ในตัวยังไม่แทน Nexus เมื่อต้องมี Maven/npm proxy แบบ air-gap",
...
T("maven-gradle", "Maven / Gradle / npm / pip (Build & Compilation)", 3, "Build & Compilation",
  ...
  ["Sonatype Nexus Repository Pro","JFrog Artifactory","Azure Artifacts","AWS CodeArtifact","Google Artifact Registry"],
  ...
  "อย่าลืมกัน disk ให้ dependency cache — เป็นสาเหตุ 'disk full' อันดับ 1 บนเครื่อง Agent; ควรตั้ง cache cleanup รายสัปดาห์ และชี้ settings.xml/npmrc ไปคลังภายใน (Nexus / GAR) ห้ามดึง Maven Central หรือ npmjs ตรงจาก Production"),
...
T("harbor", "Harbor (Private Container Registry, CNCF Graduated)", 5, "Container Registry",
  ...
  ["JFrog Artifactory Enterprise","Amazon ECR","Azure Container Registry","Google Artifact Registry","Red Hat Quay"],
  ...
  "คลัง OCI/Helm ของขั้น Store — มี Vulnerability Scan และ Cosign verification ในตัว เหมาะงานภาครัฐแบบ air-gap; ไม่แทนคลัง Maven/npm (ใช้ Nexus Repository); ต้องตั้ง Retention Policy ไม่งั้น disk โตไม่หยุด",
  oss_alt=["Zot","Nexus Repository OSS","Distribution (CNCF)","GitLab Container Registry"]),

T("nexus-repository", "Sonatype Nexus Repository OSS (Maven / npm / PyPI / Docker / Helm)", 5, "Package & Artifact Repository",
  ["package_repo","registry","version_tag","audit_trail"], "oss", "EPL-1.0", "Core",
  ["Sonatype Nexus Repository Pro","JFrog Artifactory","Azure Artifacts","AWS CodeArtifact","Google Artifact Registry"],
  4, 8, 40, 8, 16, 400, True, 4.5, "resident",
  2.00, 365, 0.20, 0.30, ["gov", "enterprise", "internal", "aiml"],
  "Sonatype system requirements: ขั้นต่ำ 4 CPU / 8 GB RAM (heap ~4 GB) สำหรับทีมเล็ก; แนะนำ 8 CPU / 16 GB เมื่อมี Docker hosted + Maven/npm proxy พร้อมกัน; blob store แยกจากดิสก์ติดตั้ง",
  "ที่หน่วยงานไทยมักเรียกว่า Nexus หรือ Apache Nexus เพราะใช้เป็นคลัง Maven ของระบบนิเวศ Apache — ผลิตภัณฑ์จริงคือ Sonatype Nexus Repository OSS (EPL-1.0) ไม่ใช่โครงการของ Apache Software Foundation (Apache Archiva ถูกยุบแล้ว); ใช้ Hosted+Proxy+Group กัน typosquatting/dependency confusion และเป็นทางหลักของงาน air-gap ที่ห้าม build ดึง Maven Central/npmjs ตรง",
  oss_alt=["GitLab Package Registry","Zot (OCI อย่างเดียว)","Verdaccio (npm)","devpi (PyPI)"],
  fit=["private", "hybrid", "local", "cloud"]),

T("zot", "Zot (OCI-native Registry, CNCF)", 5, "Container Registry",
  ["registry","artifact_sign","version_tag","audit_trail"], "oss", "Apache-2.0", "Optional",
  ["Harbor","Amazon ECR","Azure Container Registry","Google Artifact Registry"],
  1, 2, 20, 2, 4, 100, True, 0.6, "resident",
  0.40, 180, 0.10, 0.20, ["gov", "enterprise", "internal", "startup", "aiml"],
  "Zot เป็น registry ไฟล์เดียว กิน RAM ประมาณ 200-800 MB; เหมาะเครื่องเล็ก / local Kubernetes และเก็บ OCI artifact + Cosign/Notation",
  "ทางเลือกเบากว่า Harbor เมื่อต้องการแค่คลัง image/Helm ไม่มี Maven/npm — ใช้คู่กับ Nexus เมื่อต้องมีทั้งแพ็กเกจและคอนเทนเนอร์",
  oss_alt=["Harbor","Distribution (CNCF)","Nexus Repository OSS"],
  fit=["local", "private", "hybrid", "cloud"]),
...
T("azure-devops", "Azure DevOps (Cloud CI/CD Platform)", 1, "Cloud CI/CD Platform",
  ["git_scm", "webhook", "branch_protection", "pipeline", "audit_trail", "quality_gate", "deploy_strategy", "package_repo"], "commercial", "Proprietary (SaaS)", "Core",
  ...
  "CI/CD แบบ managed ของ Microsoft รวม Git, Boards, Pipelines, Azure Artifacts (Maven/npm/NuGet/PyPI), Test Plans ในที่เดียว รองรับ hybrid agent สำหรับ on-premise build",
...
T("azure-container-registry", "Azure Container Registry (ACR)", 5, "Cloud Container Registry",
...
T("aws-ecr", "Amazon Elastic Container Registry (ECR)", 5, "Cloud Container Registry",
...
T("gcp-artifact-registry", "Google Artifact Registry (GAR)", 5, "Cloud Artifact Registry",
  ["registry", "package_repo", "container_scan", "artifact_sign", "version_tag"], "commercial", "Proprietary (SaaS)", "Core",
  ...
  oss_alt=["Nexus Repository OSS", "Harbor", "Zot"], managed=True),
T("azure-kubernetes-service", "Azure Kubernetes Service (AKS)", 6, "Cloud Container Orchestration",
...
T("aws-eks", "Amazon Elastic Kubernetes Service (EKS)", 6, "Cloud Container Orchestration",
  ["orchestration", "deploy_strategy", "runtime_security", "monitoring"], "commercial", "Proprietary (SaaS)", "Core",
...
T("gcp-gke", "Google Kubernetes Engine (GKE)", 6, "Cloud Container Orchestration",
  ["orchestration", "deploy_strategy", "runtime_security", "monitoring"], "commercial", "Proprietary (SaaS)", "Core",
...
BUNDLES = {
    ...
    "store_log": dict(
        ...
        tools=["harbor", "nexus-repository", "minio", "elasticsearch", "logstash", "kibana",
    ...
    "small_worker": dict(
        ...
        tools=["jenkins-agent", "maven-gradle", "docker-buildkit", "unit-test-runner",
               ...
               "minio", "nexus-repository", "elasticsearch", "kibana", "owasp-zap", "locust", "filebeat"]),
    "ml_train": dict(
    ...
    "ml_registry": dict(
        ...
        tools=["mlflow", "llm-eval", "minio", "nexus-repository", "filebeat"]),
}
...
CONC_GROUP = {
    ...
    "nexus-repository": "resident", "zot": "resident",
...
STORAGE_CAL = {
    ...
    "nexus-repository": dict(data_daily_gb=1.50, retention_days=365,  index_overhead=0.15, growth_yr=0.30),
    "zot":              dict(data_daily_gb=0.40, retention_days=180,  index_overhead=0.10, growth_yr=0.20),
...
PROFILE_EXTRA = {
    ...
    "harbor":          ["startup"],
    "nexus-repository": ["startup"],
    "zot":             ["internal", "startup"],
    "prometheus":      ["startup"],
    "grafana":         ["startup"],
...
INSTALL = {
    ...
    "nexus-repository": _inst("binary", ["openjdk-17-jre-headless"],
                              'need_cmd nexus || install_from_mirror nexus nexus-unix.tar.gz',
                              'log "ตั้ง NEXUS_HOME + blob store แยกจาก OS disk; สร้าง hosted/proxy/group สำหรับ maven npm pypi docker"'),
    "zot": _inst("binary", None, 'need_cmd zot || install_from_mirror zot zot'),
