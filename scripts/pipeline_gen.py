...
def _app_name(ir: dict) -> str:
    pj = ir.get("project") or {}
    return sanitize_vm(pj.get("name") or "app").lower() or "app"


def _vm_csv(ir: dict) -> str:
    names = [sanitize_vm(v.get("name") or "VM") for v in (ir.get("vms") or [])]
    return ",".join(names) if names else "-"


def _jobs_csv(ir: dict) -> str:
    return ",".join(j["id"] for j in ir["jobs"] if j.get("enabled"))


def _usages_csv(ir: dict) -> str:
    parts = []
    for j in ir["jobs"]:
        if not j.get("enabled"):
            continue
        parts.append(j["id"] + ":" + (j.get("tool_id") or "bootstrap"))
    return ",".join(parts)
...
def _header(ir: dict, flavor: str) -> str:
    ...
    return (
        ...
        + " target=" + (pj.get("env") or "-") + "\n"
        "# tools=" + ",".join(ir["tools"]) + "\n"
        "# envs=" + ",".join(ir["envs"]) + "\n"
        "# vms=" + _vm_csv(ir) + "\n"
        "# jobs=" + _jobs_csv(ir) + "\n"
        "# usages=" + _usages_csv(ir) + "\n"
...
def _env_map(ir: dict) -> dict:
    ...
    return {
        ...
        "PROFILE": ir["profile"],
        "ORCHESTRATOR": ir["orchestrator"],
        "ENV_CHAIN": ",".join(ir["envs"]),
        "CICD_TOOLS": ",".join(ir["tools"]),
    }
...
def emit_gitlab(ir: dict) -> str:
    ...
    L.append("")
    L.append("variables:")
    L.append('  APP_NAME: "' + env["APP_NAME"] + '"')
    L.append('  IMAGE: "' + env["IMAGE"] + '"')
    L.append('  PROFILE: "' + env["PROFILE"] + '"')
    L.append('  ORCHESTRATOR: "' + env["ORCHESTRATOR"] + '"')
    L.append('  ENV_CHAIN: "' + env["ENV_CHAIN"] + '"')
    L.append('  CICD_TOOLS: "' + env["CICD_TOOLS"] + '"')
    L.append('  REGISTRY: "${CI_REGISTRY_IMAGE}"')
    L.append('  NEXUS_URL: "${NEXUS_URL}"')
    L.append('  DEV_URL: "${DEV_URL}"')
    L.append('  UAT_URL: "${UAT_URL}"')
    L.append('  PROD_URL: "${PROD_URL}"')
    L.append('  DR_URL: "${DR_URL}"')
    L.append('  IMAGE_REF: "${REGISTRY}/${IMAGE}:${CI_COMMIT_SHORT_SHA}"')
    L.append("")
    for j in ir["jobs"]:
        if not j["enabled"]:
            continue
...
def emit_github(ir: dict) -> str:
    ...
    for j in ir["jobs"]:
        ...
        if j["when"] == "manual":
            ...
            L.append("    environment: " + (j.get("env") or j["id"]))
        elif j.get("env"):
            L.append("    environment: " + j["env"])
        if j.get("tool_id"):
            L.append("    # tool: " + j["tool_id"] + " env=" + (j.get("env") or "-"))
        L.append("    steps:")
...
def emit_jenkins(ir: dict) -> str:
    ...
    for st in ir["stages"]:
        ...
        for j in jobs:
            if j.get("tool_id"):
                L.append("        echo 'tool=" + j["tool_id"] + " job=" + j["id"] + "'")
            for s in j["script"]:
...
def vm_script_body(ir, by_id, ids, vm_name, role):
    pkgs = []
    body = []
    self_host = []
    managed = []
    ...
    lines = [
        ...
        "# VM: " + vm_name,
        "# Role: " + (role or ""),
        "# Tools: " + (" ".join(ids) if ids else "-"),
        "# Self-host: " + (" ".join(self_host) if self_host else "-"),
        "# Managed: " + (" ".join(managed) if managed else "-"),
        "# Envs: " + (" ".join((ir or {}).get("envs") or []) or "-"),
        "# Profile: " + (((ir or {}).get("profile") or "-")),
        'HERE="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"',
        "# shellcheck disable=SC1091",
        '. "$HERE/00-common.sh"',
        'log "ติดตั้งเครื่องมือบน ' + vm_name + '"',
    ]
    if pkgs:
...
def build_install_pack(ir: dict, tools=None) -> dict:
    ...
    for i, vm in enumerate(ir.get("vms") or []):
        ...
        assigned.update(ids)
        files[fname] = vm_script_body(
            ir, by_id, ids, vm.get("name") or name, vm.get("role") or "")
    leftover = [t for t in (ir.get("tools") or []) if t not in assigned]
    extra = []
    if leftover:
        label = "unassigned" if vm_files else "selected-tools"
        fname = "install/" + label + ".sh"
        extra.append(fname)
        files[fname] = vm_script_body(
            ir, by_id, leftover, label,
            "เครื่องมือที่เลือกแล้วยังไม่ถูกจัดลง VM" if vm_files
            else "ติดตั้งตามเครื่องมือที่เลือก (ยังไม่มีผัง VM)")
    all_lines = [
        "#!/usr/bin/env sh",
        "set -eu",
        'HERE="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"',
    ...
    for f in vm_files + extra:
        all_lines.append('sh "$HERE/' + f[len("install/"):] + '"')
    if not vm_files and not extra:
        all_lines.append('log "ยังไม่มี VM และยังไม่ได้เลือกเครื่องมือ — จัดในแท็บวางแผนก่อน"')
    all_lines.append('log "ติดตั้งครบแล้ว"')
    all_lines.append("")
